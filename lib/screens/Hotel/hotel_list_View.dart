...
//  Api api = Api();
  ...
  var data;
  RangeValues _selectedRange = RangeValues(100, 10000);
                            ...
                            child: InkWell(
                              onTap: () {
                                ...
                                if (item.titel == 'Low to high') {
                                  ...
                                  hotelList.sort((a, b) => (double.parse(b
                                              .reviewScore?.scoreTotal
                                              .toString() ??
                                          '0.0'))
  ...
  Future<void> getProductFilters(BuildContext context) async {
    await filterListRequest("${baseUrl1}hotel/filters").then((value) {
      filterModel = value.data;

      setState(() {
        _selectedRange = RangeValues(
            double.parse(filterModel[0].min.toString()),
            double.parse(filterModel[0].max.toString()));
        ...
        : ListView.builder(
            ...
            itemBuilder: (context, i) {
              ...
              double rating = double.parse(
                  list?[i].reviewScore?.scoreTotal.toString() ?? '0');
